...
-- Create View showing percent population infected per country (for later visualizations)
...
SELECT location, date, total_cases, total_deaths, CONVERT(bigint, total_deaths) / total_cases * 100 AS PercentDeathPerInfected
FROM PortfolioProject..CovidDeaths
...
SELECT location, date, total_deaths, population, CONVERT(bigint, total_deaths)/population * 100 AS PercentPopulationDeath
FROM PortfolioProject..CovidDeaths
WHERE continent IS NOT NULL
ORDER BY 1, 2 
...
SELECT location, MAX(CAST(total_deaths AS bigint)) AS TotalDeathCount, MAX(CAST(total_deaths AS bigint)/population)*100 AS PercentPopulationDeath
FROM PortfolioProject..CovidDeaths
WHERE continent IS NOT NULL
GROUP BY location, population
...
SELECT location, MAX(total_cases) AS TotalCases, population, MAX(total_cases)/population * 100 AS PercentInfected
FROM PortfolioProject..CovidDeaths
...
SELECT location, MAX(total_cases) AS TotalCases, MAX(CONVERT(BIGINT, total_deaths)) AS TotalDeaths, MAX(CONVERT(BIGINT, total_deaths))/MAX(total_cases) * 100 AS PercentDeathPerInfected
FROM PortfolioProject..CovidDeaths
WHERE continent IS NULL 
	AND location <> 'World'
	AND location <> 'High Income' 
...
SELECT location, population, MAX(CAST(total_deaths AS bigint)) AS TotalDeathCount, MAX(CAST(total_deaths AS bigint))/population * 100 AS PercentPopulationDeath
FROM PortfolioProject..CovidDeaths
WHERE continent IS NULL 
	AND location <> 'World'
	AND location <> 'High Income' 
...
SELECT location, population, MAX(total_cases) AS MaxTotalCases, MAX(total_cases)/population * 100 AS PercentInfected
FROM PortfolioProject..CovidDeaths
...
SELECT location, population, MAX(CONVERT(BIGINT, total_deaths)) AS TotalDeathCount, MAX(CONVERT(BIGINT, total_deaths))/population * 100 AS PercentPopulationDeath
FROM PortfolioProject..CovidDeaths
WHERE location LIKE '%income%'
GROUP BY location, population
...
SELECT dea.location, dea.population, MAX(CAST(vac.people_vaccinated AS BIGINT)) AS PeopleVaccinated, MAX(CAST(vac.people_vaccinated AS BIGINT))/dea.population * 100 AS PercentVaccinated
...
SELECT dea.location, dea.population, MAX(CAST(vac.people_fully_vaccinated AS BIGINT)) AS PeopleFullyVaccinated, MAX(CAST(vac.people_fully_vaccinated AS BIGINT))/dea.population * 100 AS PercentFullyVaccinated
FROM PortfolioProject..CovidDeaths AS dea
JOIN PortfolioProject..CovidVaccinations AS vac
  ON dea.location = vac.location
  AND dea.date = vac.date
...
-- USE CTE to show rolling percentage of people vaccinated per country
...
-- Use Temp Table to show rolling percentage of people vaccinated per country
...
SELECT dea.location, dea.population, MAX(CONVERT(BIGINT, vac.people_fully_vaccinated)) AS PeopleVaccinated, MAX(CONVERT(BIGINT, vac.people_fully_vaccinated))/dea.population * 100 AS PercentVaccinated
FROM PortfolioProject..CovidDeaths AS dea
JOIN PortfolioProject..CovidVaccinations AS vac
  ON dea.location = vac.location
  AND dea.date = vac.date
...
SELECT dea.location, dea.date, dea.population, SUM(CONVERT(BIGINT, vac.new_people_vaccinated_smoothed)) OVER (PARTITION BY dea.location ORDER BY dea.location, dea.date) AS RollingPeopleVaccinated
FROM PortfolioProject..CovidDeaths AS dea
JOIN PortfolioProject..CovidVaccinations AS vac
  ON dea.location = vac.location
  AND dea.date = vac.date
